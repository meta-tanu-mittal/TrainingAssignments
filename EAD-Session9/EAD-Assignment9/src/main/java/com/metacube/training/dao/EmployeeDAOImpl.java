...
public class EmployeeDAOImpl implements EmployeeDAO {
	...
	private final String SQL_INSERT_EMPLOYEE = "INSERT INTO employee(first_name,last_name,dob,gender,primary_contact_no,"
			+ "secondary_contact_no,email_id,skype_id) " + "VALUES(?,?,?,?,?,?,?,?)";
	...
	private final String SQL_UPDATE_EMPLOYEE = "UPDATE employee SET first_name=?,last_name=?,dob=?,gender=?,primary_contact_no=?,"
			+ "secondary_contact_no=?,skype_id=?,password=? " + "WHERE emp_code=?";

	private final String SQL_VALID_LOGIN = "SELECT * from employee WHERE email_id=? and password=?";
	...
	public boolean updateEmployee(Employee employee) {
		return jdbcTemplate.update(SQL_UPDATE_EMPLOYEE, employee.getFirstName(), employee.getLastName(),
				...
				employee.getSecondaryContactNo(), employee.getSkypeId(), employee.getPassword(),
	...
	public Employee checkValidLogin(String emailId, String password) {
		return jdbcTemplate.queryForObject(SQL_VALID_LOGIN, new Object[] { emailId, password }, new EmployeeMapper());
	}
